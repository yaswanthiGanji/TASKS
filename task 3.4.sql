...
select  c.customer_id,c.first_name,c.last_name,c.email,c.phone_number,c.address,a.balance
...
select sum(a.balance) as total_balance_no_transactions from Accounts a
left join Transactions t on a.account_id = t.account_id
where t.transaction_id is null;
...
select customer_id from Accounts
 group by customer_id having count(distinct account_type) > 0;

-- 8. Calculate the percentage of each account type out of the total number of accounts
select account_type, count(*) * 100.0 / (select count(*) from Accounts) as percentage
from Accounts group by account_type;
-- 9. Retrieve all transactions for a customer with a given customer_id.
select t.*from Transactions t join Accounts a on t.account_id = a.account_id
where a.customer_id = '08';

-- 10. Calculate the total balance for each account type, including a subquery within the SELECT clause
select account_type, (select sum(balance) 
     from Accounts a where a.account_type = outer_query.account_type) as total_balance
